...
export const userService = {
  ...
  getAllUsers: async () => {
    try {
      ...
      const admins = (adminsRes.data?.data || adminsRes.data || []).map(u => ({ ...u, role: 'Admin', nama: u.nama_admin || u.nama || u.name }));
      const sales = (salesRes.data?.data || salesRes.data || []).map(u => ({ ...u, role: 'Sales', nama: u.nama_sales || u.nama || u.name }));
      const owners = (ownersRes.data?.data || ownersRes.data || []).map(u => ({ ...u, role: 'Owner', nama: u.nama_owner || u.nama || u.name }));
      const staff = (staffRes.data?.data || staffRes.data || []).map(u => ({ ...u, role: 'Staff Gudang', nama: u.nama_staff || u.nama || u.name }));
      const kepala = (kepalaRes.data?.data || kepalaRes.data || []).map(u => ({ ...u, role: 'Kepala Gudang', nama: u.nama_kepala || u.nama || u.name }));
  ...
  create: async (userData) => {
    ...
    if (userData.role === 'Sales') {
      endpoint = '/api/sales';
      payload.nama_sales = userData.nama || userData.name;
    } else if (userData.role === 'Owner') {
      endpoint = '/api/owners';
      payload.nama_owner = userData.nama || userData.name;
    } else if (userData.role === 'Staff Gudang') {
      endpoint = '/api/staff';
      payload.nama_staff = userData.nama || userData.name;
    } else if (userData.role === 'Kepala Gudang') {
      endpoint = '/api/kepala';
      payload.nama_kepala = userData.nama || userData.name;
    } else {
      payload.nama_admin = userData.nama || userData.name;
    }
